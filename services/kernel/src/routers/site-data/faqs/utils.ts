import { db, faq, property, propertyOwner } from "@proptryx/database";
...
export async function canAccessFaqRecord(
...
export function applyFaqListAccessScope<TQuery extends Record<string, unknown>>(
  query: TQuery,
  context: SiteDataPropertyAccessContext
) {
  if (isProptryxBroker(context)) {
