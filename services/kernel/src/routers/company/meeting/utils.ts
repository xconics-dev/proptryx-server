...
export async function findMeetingById(id: string, options?: IncludeDeletedOptions) {
  const whereClause = and(
    eq(meeting.id, id),
    options?.includeDeleted ? undefined : eq(meeting.isDeleted, false),
...
export async function validateMeetingReferences(input: MeetingReferenceInput) {
      ...
      ? db
          ...
          .where(
            and(
              ...
              eq(property.isDeleted, false),
              input.organizationId ? eq(property.organizationId, input.organizationId) : undefined
            )
          )
          .limit(1)
          .then((rows) => rows[0])
  ...
  if (input.propertyId && !checks[0]) {
    errors.push(`Property ${input.propertyId} does not exist, is deleted, or is outside scope`);
  }
...
export function canAccessMeetingAsUser(
  meetingData: Pick<
    MeetingRecord,
    "createdByUser" | "requestedByUser" | "developerId" | "occupierId"
  >,
  userId?: string | null
) {
  if (!userId) {
  ...
  return (
    meetingData.createdByUser === userId ||
    meetingData.requestedByUser === userId ||
    meetingData.developerId === userId ||
    meetingData.occupierId === userId
  );
}
